...
NAME=$(/consul/config/bin/kubectl -n onap get pod | grep -o "sparky-be[^[:space:]]*")

if [ -n "$NAME" ]; then
   if /consul/config/bin/kubectl -n onap exec -it $NAME -- ps -efww | grep 'java' | grep 'sparky' > /dev/null; then
