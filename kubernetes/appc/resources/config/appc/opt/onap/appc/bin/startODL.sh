...
function enable_odl_cluster(){
  ...
  node_name=${node[1]}
  node_index=${node[2]}
  node_list="${node_component}-${node_name}-0.{{ .Values.service.name }}-cluster.{{.Release.Namespace}}";

  for ((i=1;i<${APPC_REPLICAS};i++));
  do
    node_list="${node_list} ${node_component}-${node_name}-$i.{{ .Values.service.name }}-cluster.{{.Release.Namespace}}"
  done
