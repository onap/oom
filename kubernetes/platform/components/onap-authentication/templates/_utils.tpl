...
Usage:
{{ include "auth.realm" ( dict "dot" .Values "realm" .Values.path.to.realm) }}
*/}}
{{- define "auth.realm" -}}
{{- $dot := default . .dot -}}
...
{{ include "auth._attributes" $realm }}
{{- end -}}
...
Usage:
{{ include "auth._clients" ( dict "dot" .Values) }}
*/}}
{{- define "auth._clients" -}}
{{- $dot := default . .dot -}}
clients:
  {{- range $index, $client := $dot.clients }}
  - clientId: "{{ $client.clientId }}"
    name: "{{ $client.name }}"
    description: "{{ default "" $client.description }}"
    rootUrl: "{{ tpl (default "" $client.rootUrl) . }}"
    adminUrl: "{{ tpl (default "" $client.adminUrl) . }}"
    baseUrl: "{{ tpl (default "" $client.baseUrl) . }}"
    ...
    attributes:
      ...
      {{ toYaml $client.additionalAttributes | nindent 6 }}
      {{- end }}
...
Usage:
{{ include "auth._attributes" ( dict "dot" .Values) }}
*/}}
{{- define "auth._attributes" -}}
{{- $dot := default . .dot -}}
attributes:
  frontentUrl: {{ tpl $dot.attributes.frontendUrl . }}
  acr.loa.map: "{\"ABC\":\"5\"}"
{{- end }}
