...
*/}}
{{- define "music.repository" -}}
  {{if .Values.repositoryOverride }}
    {{- printf "%s" .Values.repositoryOverride -}}
  {{else}}
    {{- default .Values.repositorySecret .Values.global.repositorySecret -}}
  {{end}}
{{- end -}}

{{- define "music.repository.proxy" -}}
  {{if .Values.repositoryOverride }}
    {{- printf "%s" .Values.repositoryOverride -}}
  {{else}}
    {{- default .Values.repositoryProxy .Values.global.repositoryProxy -}}
  {{end}}
{{- end -}}
...
{{/*
  ...
  repositoryCred:
    ...
    mail: email (optional)
*/}}
{{- define "music.repository.secret" -}}
  {{- $repo := include "music.repository" . }}
...
{{- define "music.repository.proxy.secret" -}}
  {{- $repo := include "music.repository.proxy" . }}
  {{- $cred := .Values.global.repositoryCred }}
  {{- $mail := default "@" $cred.mail }}
  {{- $auth := printf "%s:%s" $cred.user $cred.password | b64enc }}
  {{- printf "{\"%s\":{\"username\":\"%s\",\"password\":\"%s\",\"email\":\"%s\",\"auth\":\"%s\"}}" $repo $cred.user $cred.password $mail $auth | b64enc -}}
