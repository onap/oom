...
# Import certificates to Linux SSL Truststore /etc/ssl/
cp $CERTS_DIR/*.crt $SSL_WORK_DIR
cp $MORE_CERTS_DIR/*.crt $SSL_WORK_DIR
update-ca-certificates && cp /etc/ssl/certs/ca-certificates.crt $WORK_DIR
  if [ $? != 0 ]; then
