...
bash "excuting-schema-creation" do
   code <<-EOH
     ...
     for i in {1..5}; do
       echo "=> executing /tmp/sdctool/scripts/schemaCreation.sh /tmp/sdctool/config"
       /tmp/sdctool/scripts/schemaCreation.sh /tmp/sdctool/config
       res=$?
       echo "=> /tmp/sdctool/scripts/schemaCreation.sh /tmp/sdctool/config returned status ${res}"
       if [[ ${res} == 0 ]]
       then
         break
       fi
