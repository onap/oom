...
usage()
{
    ...
    echo "Usage: registry-initialize.sh  -d chartdirectory <-n namespace override> <-r helmrelease override>"
    exit 1
}
...
LOGIN=$(kubectl -n "$NAMESPACE" get secret "${RLS_NAME}-chartmuseum-registrycred" -o jsonpath='{.data.login}' | base64 -d)
PASSWORD=$(kubectl -n "$NAMESPACE" get secret "${RLS_NAME}-chartmuseum-registrycred" -o jsonpath='{.data.password}' | base64 -d)
if [ -z "$LOGIN" ] || [ -z "$PASSWORD" ]; then
    echo "Login/Password credential cannot be retrieved"
    exit 1
fi
...
helm repo add k8s-registry http://127.0.0.1:27017 --username "$LOGIN" --password "$PASSWORD"
if [ $? -ne 0 ]; then
