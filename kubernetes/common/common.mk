...
VERSION := $(shell yq read $(ROOT_DIR)/Chart.yaml 'version' )
HELM_DEP_LIST := $(shell  helm dep list . | tr '\n' '\1')
DEP_LOCAL_DIR := $(shell echo "$(HELM_DEP_LIST)" | tr '\1' '\n' | grep file:// | cut -f3 | sed -e 's/file:\/\// /g')
DEP_LOCAL_ARCH := $(foreach dep, $(DEP_LOCAL_DIR), $(PACKAGE_DIR)/$(shell printf "$$(yq read $(dep)/Chart.yaml 'name')-$$(yq read $(dep)/Chart.yaml 'version')").tgz)
...
$(DEP_LOCAL_DIR):
	@echo "Rebuld dep [$@]"
	@if [ -f $@/Makefile ]; then\
		PACKAGE_NAME=$$(yq read $@/Chart.yaml 'name' );\
...
$(SUM_DIR)/$(PACKAGE_NAME).sum.filtered: $(SUM_DIR) $(SUM_DIR)/$(PACKAGE_NAME).sum
	@echo "\n[$@] -sum.filtered"
ifneq ($(DEP_LOCAL),)
	@grep -vE "$(DEP_LOCAL_REG)" $(SUM_DIR)/$(PACKAGE_NAME).sum > $@ || true
else
