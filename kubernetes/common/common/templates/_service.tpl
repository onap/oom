...
apiVersion: v1
kind: Service
metadata: {{ include "common.serviceMetadata" (dict "postfix" $postfix "annotations" $annotations "dot" $dot ) | nindent 2 }}
spec:
  {{- if $headless }}
  clusterIP: None
  {{- end }}
...
{{ include "common.genericService" (dict "postfix" $postfix "annotations" $annotations "dot" . "publishNotReadyAddresses" $publishNotReadyAddresses "ports" $ports "portType" $portType) }}
{{- end -}}

{{/* Create headless service template */}}
{{- define "common.headlessService" -}}
{{- $postfix := default "" .Values.service.headless.postfix -}}
...
{{ include "common.genericService" (dict "postfix" $postfix "annotations" $annotations "dot" . "publishNotReadyAddresses" $publishNotReadyAddresses "ports" $ports "portType" "ClusterIP" "headless" true ) }}
{{- end -}}
