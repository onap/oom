...
BASEDIR=$1
[ -z "$BASEDIR" ] && exit "Chart base directory provided $BASEDIR is empty"
[[ $(ls -A $BASEDIR/*tgz) ]] && echo "$BASEDIR valid" || exit "No chart package on $BASEDIR provided"

# Expose cluster port via port-forwarding
kubectl -n onap port-forward service/chart-museum 27017:8080 &
if [ $? -ne 0 ]; then
...
# Can be expanded to include all onap charts if needed
for file in $BASEDIR/dcae*tgz
do
