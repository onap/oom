...
{{- define "common._defaultPasswordStrength" -}}
  {{ if .Values.passwordStrengthOverride }}
    {{- printf "%d" .Values.passwordStrengthOverride -}}
  {{ else if .Values.global.passwordStrength }}
    {{- printf "%d" .Values.global.passwordStrength -}}
  {{ else if .Values.passwordStrength }}
    {{- printf "%d" .Values.passwordStrength -}}
  {{ else }}
    {{- printf "long" }}
  {{ end }}
{{- end -}}
...
{{- define "common.createPassword" -}}
  ...
  {{- $defaultStrength := include "common._defaultPasswordStrength" $dot -}}
