...
KEYSPACE2="CREATE KEYSPACE IF NOT EXISTS {{ .Values.janusgraph.keyspqce }} WITH REPLICATION = { 'class' : 'NetworkTopologyStrategy', '$DC_NAME': '$RC'} AND durable_writes = true;"
...
res_sdctitan=$(echo "select keyspace_name from system_schema.keyspaces;" | cqlsh -u $CASSANDRA_USER -p $CASSANDRA_PASS $CASSANDRA_IP $CASSANDRA_PORT --cqlversion={{ .Values.cassandraConfiguration.cql_version }} | grep -c {{ .Values.janusgraph.keyspqce }} 2>/dev/null)

if [ $res_sdctitan -gt 0 ]; then
    echo "$(date) --- {{ .Values.janusgraph.keyspqce }} keyspace was created"
else
    echo "$(date) --- Failed to create {{ .Values.janusgraph.keyspqce }} keyspace"
fi
