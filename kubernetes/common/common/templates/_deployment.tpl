  ...
  template:
    ...
    spec:
      ...
      {{- include "common.sidecar.init.container" . | nindent 6 }}
        {{- if .Values.initContainers.jobList }}
      - command:
        - /root/job_complete.py
        args:
          {{- range $job := .Values.initContainers.jobList }}
        - --job-name
        - {{ tpl $job $global }}
          {{- end }}
        env:
        ...
        {{- end }}
        {{- if .Values.initContainers.containerList }}
      - command:
        - /root/ready.py
        args:
          {{- range $container := .Values.initContainers.containerList }}
        - --container-name
...
{{ toYaml .Values.nodeSelector | indent 8 }}
      ...
      {{- if .Values.sideCarContainers }}
        {{- range $sideCarContainer := .Values.sideCarContainers }}
      ...
      - name: {{ tpl $volume.volumeMountRefName $global }}
        ...
        hostPath:
          path: {{ .Values.persistence.mountPath }}/{{ .Release.Name }}/{{ $volume.path }}
