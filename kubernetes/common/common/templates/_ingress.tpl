...
{{-   $port := default 80 $service.exposedPort -}}
  - name: {{ $protocol }}-{{ $port }}
    port: {{ $port }}
{{-   if $service.exposedProtocol }}
    protocol: {{ $service.exposedProtocol }}
{{-   else }}
    protocol: HTTP
{{-   end }}
    hostname: {{ include "ingress.config.host" (dict "dot" $dot "baseaddr" $baseaddr) }}
...
{{-   if eq $service.protocol "TCP" }}
        kinds:
          - kind: TCPRoute
{{-   else if eq $service.protocol "UDP" }}
        kinds:
          - kind: UDPRoute
{{-   end }}
...
{{-       else }}
{{-         if .protocol }}
  {{ include "gwapi.config.listener" (dict "dot" $dot "service" . "baseaddr" $baseaddr "protocol" (upper .protocol)) | trim }}
{{-         else }}
...
{{-     else }}
{{-       if .protocol }}
{{ include "gwapi.config.route" (dict "dot" $dot "service" . "baseaddr" $baseaddr "protocol" (upper .protocol)) | trim }}
{{-       else }}
