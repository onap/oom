...
with the given release name enter::
  ...
  > helm deploy so onap/so --version 6.0.0

.. note::
   Pass these paramater if needed: --set global.masterPassword=password
   --set global.flavor=unlimited --namespace onap
...
To deploy ONAP with this environment file, enter::
  ...
  > helm deploy local/onap -n onap -f oom/kubernetes/onap/resources/environments/onap-production.yaml
...
To see the real-time health of a deployment go to: http://<kubernetes IP/ eth0 IP>:30270/ui/
...
.. note::
   ...
   Upon successfull of this command, it will return the following output.

   > service/consul-server-ui replaced

   Now again try to access the Consul UI using this url http://<kubernetes IP/eth0 IP>:30270/ui and it must be accessible.
