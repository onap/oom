...
then
  ...
  kubectl create secret generic \
    '{{ include "common.fullname" . }}'-'{{ include "common.mariadb.secret.rootPassUID" . }}' \
    --from-literal=password=$MYSQL_ROOT_PASSWORD
fi
...
then
  ...
  kubectl create secret generic \
    '{{ include "common.fullname" . }}'-'{{ include "common.mariadb.secret.userCredentialsUID" . }}' \
    --from-literal=login=$MYSQL_USER --from-literal=password=$MYSQL_PASSWORD
fi
...
MY_REPLICA_NUMBER=$(kubectl get statefulsets {{ include "common.fullname" . }} \
  -n $NAMESPACE_ENV | grep "/"| awk '{print $2}')
...
do
    ...
    MY_REPLICA_NUMBER=$(kubectl get statefulsets \
      {{ include "common.fullname" . }} -n $NAMESPACE_ENV | grep "/" \
      | awk '{print $2}')
...
kubectl delete secret {{ include "common.fullname" . }}-db-root-password
kubectl delete secret {{ include "common.fullname" . }}-db-user-credentials
sleep 10