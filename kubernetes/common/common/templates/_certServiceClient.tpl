...
{{- if $dot.Values.global.cmpv2Enabled -}}
  ...
  {{/*# SAN's #*/}}
  {{- $dnsNames       := default ""    $certificate.dnsNames       -}}
  {{- $ipAddresses    := default ""    $certificate.ipAddresses    -}}
  {{- $uris           := default ""    $certificate.uris           -}}
  {{- $emailAddresses := default ""    $certificate.emailAddresses     -}}
  {{- $sans := concat $dnsNames $ipAddresses $uris $emailAddresses   -}}
  {{/*# Subject #*/}}
