...
{{/*
  Generate "template" metadata for Deployment / StatefulSet / ...
    The function takes several arguments (inside a dictionary)
     - .dot : environment (.)
     - .podAnnotations: podAnnotations if Values.<somestructure>.podAnnotations (dict)
     - .labels: labels to add (dict)
*/}}
{{- define "common.templateMetadata" -}}
{{- $dot := default . .dot -}}
{{- $labels := default (dict) .labels -}}
{{- $podAnnotations := default $dot.Values.podAnnotations .podAnnotations -}}
{{- if $podAnnotations }}
annotations: {{- include "common.tplValue" (dict "value" $podAnnotations "context" $) | nindent 2 }}
{{- end }}
labels: {{- include "common.labels" (dict "labels" $labels "dot" $dot) | nindent 2 }}
