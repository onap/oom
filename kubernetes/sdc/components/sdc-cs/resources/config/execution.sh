...
mkdir -p /tmp/writable-config/sdctool /tmp/writable-config/tools
...
sed -i 's/java \(.*\) -cp/java \1 -Djava.io.tmpdir=\/tmp\/writable-config\/tmp -cp/' \
    /tmp/writable-config/sdctool/scripts/schemaCreation.sh
sed -i 's/java \(.*\) -cp/java \1 -Djava.io.tmpdir=\/tmp\/writable-config\/tmp -cp/' \
    /tmp/writable-config/sdctool/scripts/janusGraphSchemaCreation.sh
sed -i 's/java \(.*\) -cp/java \1 -Djava.io.tmpdir=\/tmp\/writable-config\/tmp -cp/' \
    /tmp/writable-config/sdctool/scripts/sdcSchemaFileImport.sh

sh -x /tmp/writable-config/create-alter-dox-db.sh || exit
