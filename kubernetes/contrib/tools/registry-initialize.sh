...
while getopts ":d:n:r:p:h:" opt; do
    case $opt in
        d) BASEDIR="$OPTARG"
        ;;
        n) NAMESPACE="$OPTARG"
...
if  [ [-z "$BASEDIR"] and [-z "$HELM_CHART" ] ] ; then
    exit "Chart base directory provided $BASEDIR and helm chart from local repo is empty"
fi

if  [ [ -n "$BASEDIR"] and [ -n "$HELM_CHART" ] ]; then
    exit "Both chart base directory $BASEDIR and helm chart from local repo $HELM_CHART cannot be used at the same time "
fi
...
if  [ -n "$HELM_CHART" ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        exit "No chart package $HELM_CHART on $HELM_REPO repo"
    fi
fi
