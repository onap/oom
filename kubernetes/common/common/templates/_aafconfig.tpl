...
- name: {{ include "common.name" $dot }}-aaf-config
  image: {{ $dot.Values.global.repository }}/{{ $dot.Values.global.image }}
  imagePullPolicy: {{ $dot.Values.global.pullPolicy | default $dot.Values.pullPolicy }}
  volumeMounts:
  - mountPath: "/opt/app/osaaf"
    name: {{ include "common.fullname" $dot }}-aaf-config-vol
...
{{-     end }}
  env:
    ...
    - name: DEPLOY_FQI
    {{- include "common.secret.envFromSecret" (dict "global" $dot "uid" "aaf-deploy-creds" "key" "login") | indent 6 }}
    - name: DEPLOY_PASSWORD
    {{- include "common.secret.envFromSecret" (dict "global" $dot "uid" "aaf-deploy-creds" "key" "password") | indent 6 }}
