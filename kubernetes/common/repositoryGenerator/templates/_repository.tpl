...
{{/*
  ...
  - .Values.repository  : default image repository for all ONAP images
  - .Values.repositoryOverride : override global repository on a per image basis
*/}}
{{- define "repositoryGenerator.repository" -}}
  ...
  {{- else -}}
    {{- default $initRoot.global.repository $dot.Values.global.repository -}}
  {{- end }}
{{- end -}}
...
{{/*
  ...
  - .Values.dockerHubRepository  : default image dockerHubRepository for all dockerHub images
  - .Values.dockerHubRepositoryOverride : override global dockerHub repository on a per image basis
*/}}
{{- define "repositoryGenerator.dockerHubRepository" -}}
  ...
  {{- else }}
    {{- default $initRoot.global.dockerHubRepository $dot.Values.global.dockerHubRepository -}}
  {{- end }}
{{- end -}}
...
{{/*
  ...
  - .Values.elasticRepository  : default image elasticRepository for all images using elastic repository
  - .Values.elasticRepositoryOverride : override global elasticRepository repository on a per image basis
*/}}
{{- define "repositoryGenerator.elasticRepository" -}}
  ...
  {{- else }}
    {{- default $initRoot.global.elasticRepository $dot.Values.global.elasticRepository -}}
  {{- end }}
{{- end -}}
...
{{/*
  ...
  - .Values.googleK8sRepository  : default image dockerHubRepository for all dockerHub images
  - .Values.googleK8sRepositoryOverride : override global dockerHub repository on a per image basis
*/}}
{{- define "repositoryGenerator.googleK8sRepository" -}}
  ...
  {{- else }}
    {{- default $initRoot.global.googleK8sRepository $dot.Values.global.googleK8sRepository -}}
  {{- end }}
{{- end -}}
...
{{- define "repositoryGenerator.image.busybox" -}}
  {{- $dot := default . .dot -}}
  {{- $initRoot := default $dot.Values.repositoryGenerator .initRoot -}}
  {{- include "repositoryGenerator.dockerHubRepository" $dot }}/{{- default $initRoot.global.busyboxImage $dot.Values.global.busyboxImage -}}
{{- end -}}

{{- define "repositoryGenerator.image.curl" -}}
  {{- $dot := default . .dot -}}
  {{- $initRoot := default $dot.Values.repositoryGenerator .initRoot -}}
  {{- include "repositoryGenerator.dockerHubRepository" $dot }}/{{- default $initRoot.global.curlImage $dot.Values.global.curlImage -}}
{{- end -}}

{{- define "repositoryGenerator.image.envsubst" -}}
  {{- $dot := default . .dot -}}
  {{- $initRoot := default $dot.Values.repositoryGenerator .initRoot -}}
  {{- include "repositoryGenerator.dockerHubRepository" $dot }}/{{- default $initRoot.global.envsubstImage $dot.Values.global.envsubstImage -}}
{{- end -}}

{{- define "repositoryGenerator.image.htpasswd" -}}
  {{- $dot := default . .dot -}}
  {{- $initRoot := default $dot.Values.repositoryGenerator .initRoot -}}
  {{- include "repositoryGenerator.dockerHubRepository" $dot }}/{{- default $initRoot.global.htpasswdImage $dot.Values.global.htpasswdImage -}}
{{- end -}}

{{- define "repositoryGenerator.image.kubectl" -}}
  {{- $dot := default . .dot -}}
  {{- $initRoot := default $dot.Values.repositoryGenerator .initRoot -}}
  {{- include "repositoryGenerator.dockerHubRepository" $dot }}/{{- default $initRoot.global.kubectlImage $dot.Values.global.kubectlImage -}}
{{- end -}}

{{- define "repositoryGenerator.image.logging" -}}
  {{- $dot := default . .dot -}}
  {{- $initRoot := default $dot.Values.repositoryGenerator .initRoot -}}
  {{- include "repositoryGenerator.elasticRepository" $dot }}/{{- default $initRoot.global.loggingImage $dot.Values.global.loggingImage -}}
{{- end -}}

{{- define "repositoryGenerator.image.mariadb" -}}
  {{- $dot := default . .dot -}}
  {{- $initRoot := default $dot.Values.repositoryGenerator .initRoot -}}
  {{- include "repositoryGenerator.dockerHubRepository" $dot }}/{{- default $initRoot.global.mariadbImage $dot.Values.global.mariadbImage -}}
{{- end -}}

{{- define "repositoryGenerator.image.nginx" -}}
  {{- $dot := default . .dot -}}
  {{- $initRoot := default $dot.Values.repositoryGenerator .initRoot -}}
  {{- include "repositoryGenerator.dockerHubRepository" $dot }}/{{- default $initRoot.global.nginxImage $dot.Values.global.nginxImage -}}
{{- end -}}

{{- define "repositoryGenerator.image.postgres" -}}
  {{- $dot := default . .dot -}}
  {{- $initRoot := default $dot.Values.repositoryGenerator .initRoot -}}
  {{- include "repositoryGenerator.dockerHubRepository" $dot }}/{{- default $initRoot.global.postgresImage $dot.Values.global.postgresImage -}}
{{- end -}}

{{- define "repositoryGenerator.image.readiness" -}}
  {{- $dot := default . .dot -}}
  {{- $initRoot := default $dot.Values.repositoryGenerator .initRoot -}}
  {{- include "repositoryGenerator.repository" $dot }}/{{- default $initRoot.global.readinessImage $dot.Values.global.readinessImage -}}
{{- end -}}
...
{{- define "repositoryGenerator.alwaysFail" -}}
  {{- $dot := default . .dot -}}
  {{- $initRoot := default $dot.Values.repositoryGenerator .initRoot -}}
  {{- fail (printf "%s" $initRoot.global.repository) }}
{{- end -}}
