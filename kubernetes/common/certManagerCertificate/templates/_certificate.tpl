...
{{- if $certificate.keystore }}
  ...
  {{- $secret := (lookup "v1" "Secret" $namespace $passwordSecretRef.name) }}
{{- if $secret }}
  {{- $password = index $secret.stringData $passwordSecretRef.key }}
{{- end -}}
apiVersion: v1
kind: Secret
metadata:
...
{{- $initRoot := default $dot.Values.certManagerCertificate .initRoot -}}
{{- $subchartGlobal := mergeOverwrite (deepCopy $initRoot.global) $dot.Values.global -}}
{{- if and $subchartGlobal.cmpv2Enabled ($subchartGlobal.CMPv2CertManagerIntegration) -}}
