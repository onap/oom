...
configureEjbca() {
    ...
    ejbca.sh config cmp updatealias --alias cmpRA --key ra.certificateprofile --value CUSTOM_ENDUSER    #Cert profile name taken from certprofile filename
    ejbca.sh config cmp updatealias --alias cmpRA --key ra.endentityprofileid --value 1356531849        #ID taken from entityprofile filename
