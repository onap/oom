...
deploy_onap() {
  ...
  wget https://jira.onap.org/secure/attachment/11249/prepull_docker.sh
  ...
  if [ "$PENDING_PODS" -gt 11 ]; then
    # master/beijing only - not amsterdam
    if [ "$BRANCH" == "master" ]; then
      ./deleteAll.bash -n $ENVIRON -y
    else
      ./deleteAll.bash -n $ENVIRON
    fi
  ...
  PENDING_COUNT_SO=$(kubectl get pods -n $ENVIRON | grep mso- | grep 0/ | wc -l)
