...
mysql -h {{.Values.mariadbgalera.service.name}}.{{.Release.Namespace}} -u root -p${MYSQL_PASSWD} mysql <<-END
...
then
  mysql -h {{.Values.mariadbgalera.service.name}}.{{.Release.Namespace}} -u root -p${MYSQL_PASSWD} appcctl < ${APPC_HOME}/data/appcctl.dump
fi

if [ -f ${APPC_HOME}/data/sdnctl.dump ]
then
  mysql -h {{.Values.mariadbgalera.service.name}}.{{.Release.Namespace}} -u root -p${MYSQL_PASSWD} sdnctl < ${APPC_HOME}/data/sdnctl.dump
fi

if [ -f ${APPC_HOME}/data/sqlData.dump ]
then
  mysql -h {{.Values.mariadbgalera.service.name}}.{{.Release.Namespace}} -u root -p${MYSQL_PASSWD} sdnctl < ${APPC_HOME}/data/sqlData.dump
fi
