...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - name: operator
        image: {{ include "repositoryGenerator.githubContainerRegistry" . }}/{{ .Values.postgresOperator.controllerImages.cluster }}
        env:
        - name: CRUNCHY_DEBUG
...
metadata:
  name: {{ $dbname }}
spec:
  image: {{ include "repositoryGenerator.githubContainerRegistry" . }}/{{ .Values.postgresOperator.imagePostgres}}
  ...
  backups:
    pgbackrest:
      image: {{ include "repositoryGenerator.githubContainerRegistry" . }}/{{ .Values.postgresOperator.imagePgBackRest}}
