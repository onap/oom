...
NOT_AVALAIBLE_IMAGES=$(echo "$USED_IMAGES" | grep -vE  "$(echo "$REPO_IMAGES" | tr "\n" "|" | sed 's/|$//')")
...
for i in $USED_IMAGES; do
    ...
    if [ "$LAST_IMG" != "$TMP_IMG" ]; then
        AVALAIBLE_TAGS=$(curl -s $BASE_URL/v2/$TMP_IMG/tags/list | jq -r '.tags[]')
    fi
    if ! echo "$AVALAIBLE_TAGS" | grep "$TMP_TAG" > /dev/null; then
        NOT_AVALAIBLE_IMAGES="$NOT_AVALAIBLE_IMAGES\n$i"
    fi
    LAST_IMG="$TMP_IMG"
    printf "."
done
printf "\n"
if [ -n "$NOT_AVALAIBLE_IMAGES" ]; then
    echo "[ERROR] Only release images are allowed in helm charts."
    echo "[ERROR] Images not found in release repo:"
    echo -e "$NOT_AVALAIBLE_IMAGES"
    exit 1
fi
exit 0