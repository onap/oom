...
UEB_ADDR_IP=$(kubectl get nodes -o jsonpath='{ $.items[*].status.addresses[?(@.type=="ExternalIP")].address }' | sed 's/ /,/')
sed -i -e "s/UEB_ADDR_IP_HERE/$UEB_ADDR_IP/g" /config-init/$NAMESPACE/sdc/environments/AUTO.json
