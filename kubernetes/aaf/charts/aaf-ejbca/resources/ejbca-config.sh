...
configureEjbca() {
    ...
    ejbca.sh config cmp updatealias --alias cmpRA --key operationmode --value {{ .Values.config.ejbca.raIak }}
    ...
    ejbca.sh ra addendentity --username Node123 --dn "CN=Node123" --caname ManagementCA --password {{ .Values.config.ejbca.clientIak }} --type 1 --token USERGENERATED
    ejbca.sh ra setclearpwd --username Node123 --password {{ .Values.config.ejbca.clientIak }}
