...
{{- $dot := default . .dot -}}
{{- $constantValues := $dot.Values.cmpv2Certificate.global -}}
{{- if and $constantValues.cmpv2Enabled (not $constantValues.CMPv2CertManagerIntegration) -}}
...
{{- $sans := join "," $sansList }}
{{/*# Subject #*/}}
{{- $organization   := $constantValues.certificate.default.subject.organization        -}}
{{- $country        := $constantValues.certificate.default.subject.country             -}}
{{- $locality       := $constantValues.certificate.default.subject.locality            -}}
{{- $province       := $constantValues.certificate.default.subject.province            -}}
{{- $orgUnit        := $constantValues.certificate.default.subject.organizationalUnit  -}}
...
{{- end -}}
{{- $caName := default $constantValues.platform.certServiceClient.envVariables.caName $certificate.caName -}}
{{- $outputType := default $constantValues.platform.certServiceClient.envVariables.outputType  $certificate.outputType  -}}
{{- $requestUrl := $constantValues.platform.certServiceClient.envVariables.requestURL -}}
{{- $certPath := $constantValues.platform.certServiceClient.envVariables.certPath -}}
{{- $requestTimeout := $constantValues.platform.certServiceClient.envVariables.requestTimeout -}}
{{- $certificatesSecretMountPath := $constantValues.platform.certServiceClient.secret.mountPath -}}
{{- $keystorePath := $constantValues.platform.certServiceClient.envVariables.keystorePath -}}
{{- $keystorePassword := $constantValues.platform.certServiceClient.envVariables.keystorePassword -}}
{{- $truststorePath := $constantValues.platform.certServiceClient.envVariables.truststorePath -}}
{{- $truststorePassword := $constantValues.platform.certServiceClient.envVariables.truststorePassword -}}
...
{{- $constantValues := $dot.Values.cmpv2Certificate.global -}}
{{- if and $constantValues.cmpv2Enabled (not $constantValues.CMPv2CertManagerIntegration) -}}
{{- $certificatesSecretName := $constantValues.platform.certServiceClient.secret.name -}}
- name: certservice-tls-volume
  secret:
