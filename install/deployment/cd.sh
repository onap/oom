...
deploy_onap() {
  ...
  if [[ "$CLONE_NEW_OOM" != false ]]; then
    ...
    # usually the prepull takes up to 35 min - however hourly builds will finish the docker pulls before the config pod is finished
  fi
  ...
  cd oom/kubernetes/
  sudo make all
  sudo helm install local/onap -n dev --namespace onap
  cd ../../

  echo "wait for all pods up for 15-80 min"
  FAILED_PODS_LIMIT=0
  MAX_WAIT_PERIODS=300 # 80 MIN
  COUNTER=0
  PENDING_PODS=0
  while [  $(kubectl get pods --all-namespaces | grep 0/ | wc -l) -gt $FAILED_PODS_LIMIT ]; do
    ...
    if [ "$MAX_WAIT_PERIODS" -eq 0 ]; then
      FAILED_PODS_LIMIT=400
    fi

    # bounce SDNC - only if all other pods up - will take 2 min out of the loop
    #if [ "$MAX_WAIT_PERIODS" -eq 70 ]; then
    #  if [ "$APPLY_WORKAROUNDS" != false ]; then
    #    # master/beijing only - not amsterdam
    #    if [ "$BRANCH" == "master" ]; then
    #      ./deleteAll.bash -n $ENVIRON -a sdnc -y
    #      sleep 30
    #      ./createAll.bash -n $ENVIRON -a sdnc
    #    else
    #      ./deleteAll.bash -n $ENVIRON -a sdnc
    #      sleep 30
    #      ./createAll.bash -n $ENVIRON -a sdnc
    #    fi
    #  fi
  ...
  PENDING_COUNT_AAI=$(kubectl get pods -n $ENVIRON | grep - | grep 0/ | wc -l)
  ...
  echo "${FILEBEAT}"
  echo "sleep 2 min - to allow rest frameworks to finish"
  sleep 120
  echo "List of ONAP Modules"
  LIST_ALL=$(kubectl get pods --all-namespaces )
  echo "${LIST_ALL}"
  ...
  echo "run healthcheck for real - wait a further 3 min"
  sleep 180
  if [ "$BRANCH" == "amsterdam" ]; then
    ./ete-k8s.sh health
  else
